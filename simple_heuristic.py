...
def custom_heuristic(file_path):
    ...
    for passenger_index, passenger in df.iterrows():
        ...
        if passenger['Sex'] == 'female' and (passenger['Pclass'] == 2 or passenger['Pclass'] == 1):
            predictions[passenger_id] = 1
        elif passenger['Age'] < 18 and passenger['Pclass'] == 1:
            predictions[passenger_id] = 1
           ...
           predictions[passenger_id] = 1
        elif passenger['Sex'] == 'female' and passenger['SibSp'] == 0 and passenger['Parch'] >= 1 and passenger['Parch'] <= 3:
            predictions[passenger_id] = 1
        else:
            predictions[passenger_id] = 0
...
def main():
    ...
    # Prédictions
    predictions1 = simple_heuristic(file)
    precision1 = calculate_accuracy(predictions1, df)
    print("Your heuristic is %0.2f %% accurate. Is it 78%% or better?" % (precision1*100))

    predictions2 = complex_heuristic(file)
    precision2 = calculate_accuracy(predictions2, df)
    print("Your heuristic is %0.2f %% accurate. Is it 79%% or better?" % (precision2*100))

    predictions3 = custom_heuristic(file)
    precision3 = calculate_accuracy(predictions3, df)
    print("Your heuristic is %0.2f %% accurate. Is it 80%% or better?" % (precision3 * 100))
